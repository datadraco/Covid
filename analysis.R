...
national_df <- read.csv("https://raw.githubusercontent.com/nytimes/covid-19-data/master/us.csv")
...
state_df <- read.csv("https://raw.githubusercontent.com/nytimes/covid-19-data/master/us-states.csv")
...
num_obs_country <- nrow(national_df)
...
highest_case_locations_by_state <- county_df %>% 
  ...
  filter(date == max(date)) %>% 
  filter(cases == max(cases)) %>% 
  ungroup() %>% # the select function would add in state column so I ungrouped
  select(county_state)
...
proportion_no_deaths_df <- county_df %>% 
  group_by(state) %>% 
  filter(date == max(date)) %>%
  summarize(proportion = length(deaths[deaths == 0]) / length(deaths))
...
state_and_county_totals_df <- left_join(
  county_total_cases_df, 
  state_total_cases_df, 
  by = "date")
national_cases_df <- select(national_df, date, cases) %>% 
  rename(national_cases = cases)
total_cases_df <- left_join(
  national_cases_df, 
  state_and_county_totals_df, 
  by = "date")
...
joined_state_total_df <- state_df %>% 
  ...
  left_join(county_to_state_total_cases_df, by = c('date', 'state'))
...
state_highest_discrepancy <- total_discrepancy_df %>% 
  mutate(abs_diff = abs(state_cases - county_total)) %>% 
  group_by(state) %>% 
  summarize(total_abs_diff = sum(abs_diff)) %>% 
  filter(total_abs_diff == max(total_abs_diff)) %>% 
  pull(state)
...
# Find the day when each state saw its biggest increases in cases.
# Save the result in `state_highest_increase_df` with only the
# 'state', 'date', and 'new_cases' column, arranged by state.
# In the case of a tie, only return the most recent date.
# Save the result as `state_with_highest_increase`

state_highest_increase_df <- state_df %>% 
  arrange(date) %>% 
  group_by(state) %>% 
...
# Side note: when I try to filter for new_cases == max(new_cases) and
# date == max(date) at the same time it didn't seem to work, so I did a second
# group_by to get rid of the tie, why is that necessary?

# Find the national average deaths per day since the first reported death.
# Save the result as `national_daily_death_avg`

days_of_death <- nrow(filter(national_df, deaths > 0))
total_death <- national_df %>% 
  filter(deaths == max(deaths)) %>% 
  pull(deaths)
national_daily_death_avg <- total_death / days_of_death
